...
struct network{
	double inp[N];
	double weight[N];
	double quadWeight[N][N];
	double sum;
	double change[N];
	double quadChange[N][N];
...
double network::pass(gamestate gs){
	...
	for(i=0; i<N-1; i++){
		for(j=i+1; j<N; j++){
			sum += inp[i] * inp[j] * quadWeight[i][j];
		}
	}
	return sigmoid(sum);
}
...
move network::makemove(gamestate gs, int team){
	...
	for(i=0; i<boardx; i++){
		for(j=0; j<boardy; j++){
			if(gs.board[i][j] == 0){
				gs.board[i][j] = team;
				prop = exp(3*team*invsigmoid(pass(gs)));
				gs.board[i][j] = 0;
	...
	double randVal = (double)(rand() * rand() % RAND_MAX) / RAND_MAX * sum;
	double parsum = 0;
	for(i=0; i<boardx; i++){
		for(j=0; j<boardy; j++){
			if(gs.board[i][j] == 0){
...
void network::descent(){
	...
	for(g=0; g<numgames; g++){
		for(m=0; m<=games[g].moves; m++){
			...
			for(i=0; i<N-1; i++){
				for(j=i+1; j<N; j++){
					quadChange[i][j] += inp[i] * inp[j] * dsigmoid(sum) * 2*(guess-expect);
				}
			}
		}
	}
	for(i=0; i<N; i++){
		weight[i] -= change[i] / numgames * rate;
	}
	for(i=0; i<N-1; i++){
		for(j=i+1; j<N; j++){
			quadWeight[i][j] -= quadChange[i][j] / numgames / N * rate;
		}
	}
}
